# Arquivo: processing/financeiro.py
import os
import re
import pandas as pd
import mariadb
...
def extrair_dados_do_excel(caminho_arquivo, lojas_map):
    ...
    for num_linha in LINHAS_PARA_EXTRAIR:
        ...
        dados_linha = {
            ...
            "percentual_geral": limpar_percentual(df_excel.iat[idx_linha, col_idx_map['E']]),
            "valor_media_loja_6m": limpar_valor(df_excel.iat[idx_linha, col_idx_map['F']]),
            "percentual_media_loja_6m": limpar_percentual(df_excel.iat[idx_linha, col_idx_map['G']]),
            "valor_media_faixa_faturamento_6m": limpar_valor(df_excel.iat[idx_linha, col_idx_map['H']]),
            "percentual_media_faixa_faturamento_6m": limpar_percentual(df_excel.iat[idx_linha, col_idx_map['I']]),
            "valor_media_febrafar_6m": limpar_valor(df_excel.iat[idx_linha, col_idx_map['J']]),
            "percentual_media_febrafar_6m": limpar_percentual(df_excel.iat[idx_linha, col_idx_map['K']])
        }
...
def processar_arquivo(caminho_arquivo, lojas_map, conn):
    ...
    cursor = conn.cursor()
    df_arquivo = extrair_dados_do_excel(caminho_arquivo, lojas_map)

    if df_arquivo is None or df_arquivo.empty:
        print(f"Nenhum dado extraído de {os.path.basename(caminho_arquivo)}.")
        return

    # Reordena e limpa os dados
    ordem_colunas_db = [
    ...
    df_arquivo = df_arquivo[ordem_colunas_db]
    df_arquivo = df_arquivo.replace({np.nan: None})
    ...
    sql_insert = f"""
        ...
        ON DUPLICATE KEY UPDATE
            loja_nome=VALUES(loja_nome), valor_geral=VALUES(valor_geral), percentual_geral=VALUES(percentual_geral),
            valor_media_loja_6m=VALUES(valor_media_loja_6m), percentual_media_loja_6m=VALUES(percentual_media_loja_6m),
    ...
    dados_para_inserir = [tuple(row) for row in df_arquivo.to_numpy()]
    
    try:
        cursor.executemany(sql_insert, dados_para_inserir)
        conn.commit()
        print(f"  -> SUCESSO! {cursor.rowcount} registros de {os.path.basename(caminho_arquivo)} inseridos/atualizados.")
    except mariadb.Error as e:
        print(f"  -> ERRO ao inserir dados de {os.path.basename(caminho_arquivo)}: {e}")
        conn.rollback()

def main():
    # Esta função agora processa todos os arquivos na pasta, útil para execuções manuais ou de recuperação
    conn = None
    try:
        ...
        arquivos_excel = [f for f in os.listdir(PASTA_DOS_ARQUIVOS_EXCEL) if f.endswith('.xlsx') and "financeiro" in f.lower() and not f.startswith('~$') and "performance" not in f.lower()]
        if not arquivos_excel:
            print(f"Nenhum arquivo Excel financeiro encontrado em '{PASTA_DOS_ARQUIVOS_EXCEL}'")
            return
        ...
        print(f"ERRO de banco de dados no main: {e}")
        sys.exit(1)
    finally:
